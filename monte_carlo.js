...
const N = 1000;
const PROFILES = ['beginner', 'medium', 'expert'];
const allResults = {};

console.log('\n[BAŞLIYOR] 3000 oyun simüle ediliyor (1000 × 3 profil)...\n');

for (const profile of PROFILES) {
  const label = { beginner: 'YENİ BAŞLAYAN', medium: 'ORTA SEVİYE', expert: 'UZMAN' }[profile];
...
function analyze(runs) {
  ...
  return { total, wins: wins.length, losses: losses.length, lossBy, padishahs, avgYr, avgEvham };
}
